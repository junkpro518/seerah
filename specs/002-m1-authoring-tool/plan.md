...
**ملاحظة حوكمة (تحتاج انتباه المالك):** الدستور (قسم سير التطوير) يقول "النطاق الحالي: م٠ فقط — لا تُبنى مراحل لاحقة". المالك **أذن صراحةً ببدء م١**، فهذا تجاوز مُعتمَد من المالك. **يُوصى بتعديل دستوري (PATCH/MINOR)** يحدّث سطر النطاق ليشمل م١ — لإبقاء الدستور مطابقًا للواقع. لا يحجب هذا الخطة لكنه مدوّن للاعتماد.

**نقطة للتأكيد (أمن، غير حاجبة):** تخزين الرموز الاحتياطية مجزّأةً في `auth.users` app_metadata يحقّق قرار "رموز احتياطية" بلا تغيير مخطط؛ إن فضّل المالك جدولًا مخصّصًا، فهو **ترحيل منفصل في مسار م٠** يتطلّب إذنه. (انظر research.md.)
